...
import { OTLPExportLogsServiceRequest, OTLPLogRecord } from "../types/otlp";

const SEVERITY_LEVELS: LogLevel[] = [
  LogLevel.ERROR,
  LogLevel.WARN,
  LogLevel.INFO,
  LogLevel.DEBUG,
 ...
 */
export function generateSyntheticOTLP(
  logCount: number,
  ...
  const resourceLogs = SERVICE_NAMES.map((serviceName, serviceIndex) => {
    ...
    const recordCount =
      serviceIndex === SERVICE_NAMES.length - 1
