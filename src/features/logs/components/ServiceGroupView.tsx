...
export function ServiceGroupView({ groups }: ServiceGroupViewProps) {
  ...
  const virtualizer = useVirtualizer({
    ...
    estimateSize: (index) => {
      return flattenedItems[index].type === "header"
        ? HEADER_ESTIMATE_PX
        : ROW_ESTIMATE_PX;
    },
              ...
              >
                {item.type === "header" ? (
                  <div className="pt-4 pb-2">
                    ...
                    {!item.isExpanded && (
                      <div className="h-0 border-b border-gray-800/60 rounded-b-lg -mx-[1px]" />
                    )}
                  </div>
                ) : (
                  <div
                    className={`border-x border-gray-800/40 bg-black ${item.isLast ? "rounded-b-lg border-b mb-4" : ""}`}
                  >
